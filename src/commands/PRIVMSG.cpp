...
void privMsgCmd(commandData& cmd, Server& server, Client& client)
{
	...
	for (size_t i = 0; i < receivers.size(); i++)
	{
		if (receivers[i][0] == '#' || receivers[i][0] == '&')
		{
			...
			channel->broadcastMessage(&client, RPL_PRIVMSG(client.getNickname(), receivers[i], cmd.arguments[1]));
		}
		else
		{
			...
			Server::sendReply(RPL_PRIVMSG(client.getNickname(), receivers[i], cmd.arguments[1]), receiver->getFd());
		}
	}
}